...
cli.responders.listLogs = function() {
    const ls = childProcess.spawn('powershell', ['ls', './.logs']);
    ls.stdout.on('data', function(dataObject) {
        ...
        logFileNames.forEach(function(logFileName) {
            if (typeof (logFileName) == 'string' && logFileName.length > 0 && logFileName.indexOf('b64') > -1) {
                console.log(logFileName.trim().split('.')[0].split(' ').slice(-1)[0]);
            }
        });
    });
}

// List logs using _logs.list
